...
const mockAnalysis = {
  ...
  recentSideEffects: [
    { effect: "Dry cough", reportDate: "2023-05-10", severity: "Mild", count: 15 },
    { effect: "Dizziness", reportDate: "2023-05-08", severity: "Moderate", count: 8 },
    { effect: "Headache", reportDate: "2023-05-05", severity: "Mild", count: 12 },
    { effect: "Fatigue", reportDate: "2023-05-03", severity: "Mild", count: 7 },
    { effect: "Nausea", reportDate: "2023-05-01", severity: "Moderate", count: 5 },
  ],
  commonSideEffects: ["Dizziness", "Headache", "Cough", "High potassium levels"],
  rareSideEffects: ["Angioedema", "Liver failure", "Pancreatitis"],
  interactions: [
    { drug: "NSAIDs (e.g., ibuprofen)", severity: "Moderate", effect: "May reduce effectiveness of Lisinopril" },
    { drug: "Potassium supplements", severity: "Severe", effect: "Risk of hyperkalemia (high potassium levels)" },
    { drug: "Lithium", severity: "Moderate", effect: "May increase lithium levels in the blood" },
    { drug: "Diuretics", severity: "Mild", effect: "May enhance blood pressure-lowering effect" },
    { drug: "Aliskiren", severity: "Severe", effect: "Increased risk of kidney problems and high potassium levels" },
  ],
  precautions: [
  ...
  patientReviews: [
    ...
    { rating: 2, comment: "Caused dizziness, had to switch to an alternative." },
  ],
  efficacyData: [
    { month: 1, efficacy: 70 },
    { month: 2, efficacy: 75 },
...
export function PrescriptionAnalyzer({ darkMode }) {
  ...
  useEffect(() => {
    if (analysisResult && interactionsRef.current) {
      renderInteractionsNetwork(analysisResult.interactions, interactionsRef.current, darkMode)
    }
  ...
  const filteredSections = Object.keys(analysisResult || {}).filter(
    (key) =>
      typeof analysisResult[key] === "string" && analysisResult[key].toLowerCase().includes(searchTerm.toLowerCase()),
  )
  ...
  const renderWeatherInfo = () => {
    ...
    return (
      <div className={`mt-4 p-4 rounded-lg ${darkMode ? "bg-gray-700" : "bg-blue-50"}`}>
        <h4 className={`text-lg font-semibold mb-2 ${darkMode ? "text-blue-300" : "text-blue-600"}`}>
          Current Weather
        </h4>
        <div className="flex items-center space-x-4">
          {currentWeather.condition === "Sunny" ? (
            <Sun className={`w-8 h-8 ${darkMode ? "text-yellow-300" : "text-yellow-500"}`} />
          ) : currentWeather.condition === "Rainy" ? (
            <CloudRain className={`w-8 h-8 ${darkMode ? "text-blue-300" : "text-blue-500"}`} />
          ) : (
            <Wind className={`w-8 h-8 ${darkMode ? "text-gray-300" : "text-gray-500"}`} />
          )}
          <div>
        ...
        <p className={`mt-2 text-sm ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
          Remember to stay hydrated and protect yourself from the sun while taking this medication.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className={`max-w-4xl mx-auto p-6 bg-white ${darkMode ? "dark:bg-gray-800" : ""} rounded-lg shadow-lg`}>
      <h2 className={`text-3xl font-bold mb-6 text-center text-blue-600 ${darkMode ? "dark:text-blue-400" : ""}`}>
          ...
          <div className="flex items-center justify-center w-full">
            <label
              ...
              className={`flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 ${
                darkMode ? "dark:hover:bg-gray-800 dark:bg-gray-700" : "hover:bg-gray-100"
            ...
            >
              ...
              <button
                ...
                className={`px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-300 ${
                  darkMode ? "dark:bg-blue-600 dark:text-white dark:hover:bg-blue-700" : ""
                }`}
        ...
        {analysisResult && (
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h3 className={`text-2xl font-semibold text-gray-800 ${darkMode ? "dark:text-gray-200" : ""}`}>
                ...
                <div className="mt-2">
                  ...
                  <ul className="list-disc list-inside">
                    {filteredSections.map((section) => (
                      <li key={section} className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
                        {section}
                      </li>
                    ))}
                  </ul>
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <InfoCard label="Generic Name" value={analysisResult.genericName} darkMode={darkMode} />
                  <InfoCard label="Drug Class" value={analysisResult.drugClass} darkMode={darkMode} />
                  <InfoCard label="Manufacturer" value={analysisResult.manufacturer} darkMode={darkMode} />
                  <InfoCard label="Approval Date" value={analysisResult.approvalDate} darkMode={darkMode} />
                  <InfoCard label="Age Range" value={analysisResult.ageRange} darkMode={darkMode} />
                  <InfoCard
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <InfoCard label="Dosage" value={analysisResult.dosage} darkMode={darkMode} />
                  <InfoCard label="Frequency" value={analysisResult.frequency} darkMode={darkMode} />
                  <InfoCard label="Duration" value={analysisResult.duration} darkMode={darkMode} />
                </div>
                {/* New: Time of day selector */}
                <div className="mt-4">
                  <h5 className={`text-lg font-semibold mb-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
              ...
              <div>
                ...
                <div className="flex items-center justify-center">
                  <QualityScoreGauge score={analysisResult.qualityScore} darkMode={darkMode} />
                </div>
              </div>
              ...
              <div>
                ...
                {analysisResult.flaggedBy.length > 0 ? (
                    ...
                    {analysisResult.flaggedBy.map((flag, index) => (
                      <li
                        ...
                        className={`flex items-start p-3 rounded-lg ${
                          darkMode ? "bg-gray-700 text-yellow-400" : "bg-yellow-50 text-yellow-700"
                        }`}
                      >
                  ...
                  <p className={`text-green-600 ${darkMode ? "dark:text-green-400" : ""}`}>
              ...
              <div>
                ...
                <SideEffectsChart sideEffects={analysisResult.recentSideEffects} darkMode={darkMode} />
                    ...
                    {analysisResult.interactions.map((interaction, index) => (
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <span className="ml-2 font-semibold">{interaction.drug}</span>
                        </div>
              ...
              <div>
                ...
                {showEfficacyChart && <EfficacyChart data={analysisResult.efficacyData} darkMode={darkMode} />}
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <InfoCard
                    ...
                    value={analysisResult.environmentalImpact.productionFootprint}
                    darkMode={darkMode}
                  />
                  <InfoCard
                  ...
                  <InfoCard
                    ...
                    value={analysisResult.environmentalImpact.recyclingOptions}
                    darkMode={darkMode}
                  />
                </div>
              </div>
            ...
            <div className="mt-8 flex justify-center space-x-4">
              ...
              <button
                className={`px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition duration-300 ${
                  darkMode ? "dark:bg-green-600 dark:text-white dark:hover:bg-green-700" : ""
                }`}
              >
                Schedule Pickup
              </button>
...
const ExpandableSection = ({ title, items, darkMode, expandedSections, toggleSection }) => (
  <div>
    <button
    ...
    >
      <span>{title}</span>
      {expandedSections.includes(title) ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
    </button>
    <AnimatePresence>
...
const QualityScoreGauge = ({ score, darkMode }) => {
  ...
  return (
    <div className="relative w-48 h-48">
      ...
      <div className={`text-center mt-2 ${darkMode ? "text-gray-300" : "text-gray-600"}`}>Quality Score</div>
    </div>
  )
}
...
const renderInteractionsNetwork = (interactions, container, darkMode) => {
  ...
  const nodes = [{ id: "Lisinopril", group: 0 }, ...interactions.map((i, index) => ({ id: i.drug, group: 1 }))]
  const links = interactions.map((i, index) => ({ source: "Lisinopril", target: i.drug, value: i.severity }))
