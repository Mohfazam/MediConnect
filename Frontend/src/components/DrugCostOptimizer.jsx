...
// Update the Lisinopril data in MEDICATIONS_DATABASE to match mockAnalysis
...
export function DrugCostOptimizer() {
  ...
  const chartData = selectedDrug
    ? {
        labels: selectedDrug.costHistory.map((item) => item.month),
        datasets: [
          {
            label: "Monthly Cost",
            data: selectedDrug.costHistory.map((item) => item.amount),
                ...
                <div className="absolute w-full mt-2 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-500">
                              {mockAnalysis.frequency} • Analyzed on {mockAnalysis.recentSideEffects[0].reportDate}
                            </div>
                          </div>
                        </div>
                        <button
                  ...
                  {searchTerm && SIMILAR_MEDICATIONS[searchTerm] && (
                    <div className="p-2">
                      ...
                      {getSimilarMedications(searchTerm).map((med, index) => (
                        <button
                          key={index}
                          onClick={() => {
                            handleSearch(med)
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center">
                  ...
                  <div>
                    <span className="font-medium">Use Latest Prescription</span>
                    <p className="text-sm text-gray-500">
                      Analyze costs for {mockAnalysis.drugName} {mockAnalysis.dosage}
                    </p>
                  </div>
                </div>
          ...
          >
            <div className="flex items-center">
              ...
              <p className="text-green-700">
                Potential savings of up to {selectedDrug?.generic.savings}% found for {searchTerm}!
              </p>
            </div>
          </motion.div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div>
              ...
              <div className="space-y-2">
                <p className="text-gray-600">Brand Name: {selectedDrug.brandName}</p>
      ...
      <AnimatePresence mode="wait">
        {selectedDrug && (
          ...
          >
            {/* Insurance Panel */}
            {activeTab === "insurance" && (
              <InsurancePanel insurance={selectedDrug.insurance} drugName={selectedDrug.brandName} />
            )}

            {/* Generic Panel */}
            {activeTab === "generic" && (
              <GenericPanel generic={selectedDrug.generic} brandName={selectedDrug.brandName} />
            )}

            {/* Pharmacy Panel */}
            {activeTab === "pharmacy" && (
              <PharmacyPanel pharmacies={selectedDrug.pharmacyPrices} selectedStrength={selectedStrength} />
            )}
            ...
            {activeTab === "subscription" && (
              <SubscriptionPanel options={selectedDrug.subscriptionOptions} selectedStrength={selectedStrength} />
            )}

            {/* Patient Assistance Panel */}
            {activeTab === "assistance" && (
              <AssistancePanel assistance={selectedDrug.patientAssistance} drugName={selectedDrug.brandName} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {!selectedDrug && !isLoading && searchTerm && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center p-8">
          ...
          <p className="text-gray-600">
            We couldn't find any information for "{searchTerm}". Please check the spelling or try another medication.
          </p>
        </motion.div>
      )}
    </div>
...
const TabButton = ({ id, icon: Icon, label, alert, onClick, isActive }) => (
  <button
    ...
    className={`flex items-center justify-between w-full p-4 rounded-lg transition-all transform hover:scale-105 ${
      isActive ? "bg-blue-600 text-white shadow-lg" : "bg-white text-gray-700 hover:bg-blue-50 shadow"
    }`}
  >
    <div className="flex items-center">
    ...
    {alert && <span className="bg-red-500 text-white px-2 py-1 rounded-full text-xs">Save 85%</span>}
  </button>
)
...
const InsurancePanel = ({ insurance, drugName }) => (
  <div className="space-y-6">
    <h3 className="text-2xl font-bold text-gray-800 mb-4">Insurance Coverage Details</h3>
    <div
      className={`p-6 rounded-xl ${
        insurance.covered ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"
      }`}
    >
      <div className="flex items-start">
        ...
        <div className="space-y-4">
          <h4 className="text-lg font-semibold">
            {insurance.covered ? "Covered by Insurance" : "Not Covered by Insurance"}
          </h4>
          {insurance.covered && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-gray-800">{insurance.coverage}%</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Tier</p>
                  <p className="text-xl font-bold text-gray-800">{insurance.tier}</p>
                </div>
              </div>
              <div className="bg-blue-50 p-4 rounded-lg">
                <h5 className="font-medium text-blue-800 mb-2">Network Providers</h5>
                <div className="flex flex-wrap gap-2">
                  {insurance.networkProviders.map((provider, index) => (
                    <span key={index} className="px-3 py-1 bg-white rounded-full text-sm text-blue-600">
                      {provider}
                    </span>
                  ))}
                </div>
...
const GenericPanel = ({ generic, brandName }) => (
  <div className="space-y-6">
    <h3 className="text-2xl font-bold text-gray-800 mb-4">Generic Alternative Options</h3>
    {generic.available ? (
      <div className="space-y-6">
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
          <h4 className="text-lg font-semibold text-blue-800 mb-4">Generic Information</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
        ...
        <div className="bg-white border rounded-xl p-6">
          <h4 className="text-lg font-semibold text-gray-800 mb-4">Price Comparison</h4>
        ...
        <div>
          <h4 className="text-lg font-semibold text-gray-800 mb-4">Available Manufacturers</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div className="flex items-center p-4 bg-yellow-50 rounded-lg">
        ...
        <p className="text-yellow-700">No generic alternatives available for {brandName}.</p>
      </div>
    )}
  </div>
)
...
const PharmacyPanel = ({ pharmacies, selectedStrength }) => (
  <div className="space-y-6">
    <h3 className="text-2xl font-bold text-gray-800 mb-4">Pharmacy Price Comparison</h3>
...
const DiscountsPanel = ({ discounts, additionalSavings }) => (
  <div className="space-y-6">
    <h3 className="text-2xl font-bold text-gray-800 mb-4">Available Discount Programs</h3>
        ...
        <div key={index} className="bg-white border rounded-xl p-6 hover:shadow-lg transition-shadow">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-green-600 font-medium">Save up to {program.discount}%</p>
          ...
          <div className="mt-4 pt-4 border-t">
            <p className="text-sm text-gray-600">
              <span className="font-medium">How to use:</span> {program.howToUse}
            </p>
    ...
    <div className="mt-8">
      <h4 className="text-xl font-semibold text-gray-800 mb-4">Additional Savings Options</h4>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {additionalSavings.manufacturerCoupons.available && (
          <div className="bg-white border rounded-lg p-4">
            ...
            <p className="text-green-600 font-bold">Save up to {additionalSavings.manufacturerCoupons.maxSavings}%</p>
            <p className="text-sm text-gray-600 mt-2">
              Expires: {additionalSavings.manufacturerCoupons.expirationDate}
            </p>
            <p className="text-sm text-gray-500 mt-1">{additionalSavings.manufacturerCoupons.terms}</p>
          </div>
        )}
        ...
        {additionalSavings.bulkPurchase.available && (
          <div className="bg-white border rounded-lg p-4">
            ...
            <p className="text-green-600 font-bold">Save {additionalSavings.bulkPurchase.savings}%</p>
            <p className="text-sm text-gray-600 mt-2">
              Minimum quantity: {additionalSavings.bulkPurchase.minQuantity} days supply
            </p>
          </div>
        )}
        ...
        {additionalSavings.mailOrder.available && (
          <div className="bg-white border rounded-lg p-4">
            ...
            <p className="text-green-600 font-bold">Save {additionalSavings.mailOrder.savings}%</p>
            <p className="text-sm text-gray-600 mt-2">Shipping time: {additionalSavings.mailOrder.shippingTime}</p>
          </div>
        )}
      </div>
    </div>
...
const AssistancePanel = ({ assistance, drugName }) => (
  <div className="space-y-6">
    <h3 className="text-2xl font-bold text-gray-800 mb-4">Patient Assistance Programs</h3>
    ...
    <div className="bg-blue-50 rounded-lg p-6">
      <h4 className="text-lg font-semibold text-blue-800 mb-4">Medicare Coverage Information</h4>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-lg p-4">
          <p className="text-sm text-gray-600">Part D Coverage</p>
          <p className="font-medium text-blue-600">{assistance.medicareInfo.partD}</p>
        </div>
        <div className="bg-white rounded-lg p-4">
          <p className="text-sm text-gray-600">Donut Hole</p>
          <p className="font-medium text-blue-600">{assistance.medicareInfo.donutHole}</p>
        </div>
        <div className="bg-white rounded-lg p-4">
          <p className="text-sm text-gray-600">Catastrophic Coverage</p>
          <p className="font-medium text-blue-600">{assistance.medicareInfo.catastrophicCoverage}</p>
        </div>
      </div>
    </div>
  </div>
